...
const compress = (options, flags) => {
  ...
  // console.log(files)
    ...
    Promise.allSettled(promises).then((results) => {
      ...
      log.success(`Images compressed successfully in ${duration}ms`);
      
      // took out / missing cb from the function construction
      // if(cb != undefined){ cb(); }
    });
...
for(let i = 1 ; i <= 3; i++){
  ...
  if (flags.type === 'none'){
    outputExtension = `.${pathDetails.extension}` //add the dot after the split
  }else{
  ...
  if(outputExtension === '.jpg' ){
    ...
    s.jpeg({quality: Number(flags.quality)})
    output = `${srcPath}/pages/${pathDetails.path}${flags.directory}/${pathDetails.filename}-w${sizeCalc}.jpeg`
