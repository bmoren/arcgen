...
//break the url out into it's own thing so that we can use it inside the build page to later build the page refs as part of that process so that we can more easily get the media without havig to have a bunch of loops.
...
const _buildPageDefs = (file, { srcPath }) => {
    ...
    pageDefs.push({...pageData.attributes, url, media})
