...
//this utilizes a callback inside the compress function, it's not ideal and really should prob use a promise, but I just cant figure out how that works.
// const build = (options = {}, flags) => {
//     //compress the images flag is true
//   if(flags.i === true){
//       compress(options,flags,function(){
//       buildMain(options,flags)
//     });
//   }else{
//     buildMain(options,flags)
//   }
...
/*
//not implemented, come back to this later maybe?
//everything working with spaced paths now so maybe it's not a big deal?
*/
 ...
 * copies all media recurcively to appropriate directory, skipping markdown and system files.
 ARCHIVED FUNCTION from pre-path cleaner version
 */
...
const _copyMediaClean = ({srcPath, outputPath}) => {
  ...
  files.forEach(function(file,i){
    ...
    let destination = _stringPathCleaner(`/${file}`)

    // console.log(file, " > ", to);
    fse.copySync(`${srcPath}/pages/${file}`, `${outputPath}/${destination}`);
  })
...
const _buildMediaDef = (file, { srcPath }) => {
  ...
  //maybe should switch this to image types only so that it's more known whats going to be here and less parsing on the template
  let media = glob.sync('*.!(md|ejs|html|pdf)', { cwd: mediaSrc });
...
const _buildPage = (file, { srcPath, outputPath, site }) => {
  ...
  const templateConfig = {
    pages: pageDefs,
    site,
    page: {...pageData.attributes, media, url }
  };
  ...
  switch (fileData.ext) {
    case '.md':
      ...
      break;
    //remove ejs functionlity from data pages, it's no longer supported
    // case '.ejs':
